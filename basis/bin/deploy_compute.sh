...
then
  rsync -av -e "ssh -o StrictHostKeyChecking=no -i id_starter_rsa" target/compute/* opc@$COMPUTE_IP:.
else
  scp -r -o StrictHostKeyChecking=no -i id_starter_rsa target/compute/* opc@$COMPUTE_IP:/home/opc/.
fi
