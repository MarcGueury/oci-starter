...
then
  rsync -av -e "ssh -o StrictHostKeyChecking=no -i id_starter_rsa" src/db_src opc@$BASTION_IP:.
else
  scp -r -o StrictHostKeyChecking=no -i id_starter_rsa src/db_src opc@$BASTION_IP:/home/opc/.
fi
